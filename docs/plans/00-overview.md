...
> 本目录 `docs/plans/` 下 11 份计划按依赖顺序编号；本文是导航与全局约定。
...
| 10 | `10-packaging-distribution.md` | 打包与分发：mega 包 + `dsh plugin add` 一键安装（方案已定稿，P0–P3） |
...
01 数据模型
   └─▶ 02 cm-flow 扩展（依赖 01 的表）
          ...
          │                             └─▶ 09 测试验收（横跨全部）
          └─▶ 10 打包分发（依赖 01–09 全部交付；方案已定稿，见文件内 P0–P3）
```

**必须顺序**：01 → 02 → 04 → 05/06/07 → 08 → 09。03 可与 02 并行。每份计划都标注「前置」。10 在 09 之后或与之并行推进。
