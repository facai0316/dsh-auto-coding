...
配置页下方还有 **worker 运行配置**卡片（保存后下个 tick ≤10s 生效）：

- **时段**：勾选「启用时段限制」后设置窗口（起=止视为不限；结束早于起始
  视为**跨天夜间窗口**，如 22:00→06:00）。可进一步勾选**仅某些阶段限时段**——
  勾选的阶段在窗口外不领取 / 不续跑 / 不重试（链上跑到该阶段会停下，进窗后
  自动接续）；未勾选的阶段 24h 可跑，全不勾 = 完全不限。
- **并发 / 默认模型 / 每阶段模型**：全局并发预算（1–8）与各阶段子会话的
  提供商 / 模型 / 最大 tokens 覆盖（留空继承）。
