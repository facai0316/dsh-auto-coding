...
const REVIEW_KINDS = ["review", "reply"];
/** 审核单状态。 */
...
function normalizeWorkerConfig(input) {
	...
	for (const [category, raw] of Object.entries(stages)) {
		...
		normalizedStages[category] = {
			...stage.provider !== null && stage.provider !== void 0 ? { provider: String(stage.provider) } : {},
			...stage.model !== null && stage.model !== void 0 ? { model: String(stage.model) } : {},
			...stage.maxTokens !== null && stage.maxTokens !== void 0 ? { maxTokens: Number(stage.maxTokens) } : {}
		};
	}
	const clampHour = (candidate, fallback) => {
		const parsed = Number(candidate);
		return Number.isFinite(parsed) ? Math.min(23, Math.max(0, Math.floor(parsed))) : fallback;
	};
	const optionalText = (candidate) => candidate === null || candidate === void 0 || candidate === "" ? void 0 : String(candidate);
	const optionalTokens = (candidate) => {
		if (candidate === null || candidate === void 0 || candidate === "") return void 0;
		const parsed = Number(candidate);
		return Number.isFinite(parsed) && parsed > 0 ? Math.floor(parsed) : void 0;
	};
	const clampConcurrency = (candidate) => {
		const parsed = Number(candidate);
	...
	return {
		...
		...optionalText(value.defaultModel) !== void 0 ? { defaultModel: optionalText(value.defaultModel) } : {},
		...optionalText(value.defaultProvider) !== void 0 ? { defaultProvider: optionalText(value.defaultProvider) } : {},
		...optionalTokens(value.defaultMaxTokens) !== void 0 ? { defaultMaxTokens: optionalTokens(value.defaultMaxTokens) } : {}
	};
}
/**
...
function assertStatus(value) {
	if (typeof value === "string" && REQUIREMENT_STATUSES.includes(value)) return value;
	throw new Error(`未知需求状态 ${JSON.stringify(value)}（合法值：${REQUIREMENT_STATUSES.join(", ")}）`);
}
function assertRecordStatus(value) {
	if (typeof value === "string" && RECORD_STATUSES.includes(value)) return value;
...
function rowToView(row) {
	return {
		...
		description: row.description === null || row.description === void 0 ? null : String(row.description),
		status: assertStatus(row.status),
		projectId: row.project_id === null || row.project_id === void 0 ? null : String(row.project_id),
...
function recordRowToView(row) {
	return {
		...
		result: row.result === null || row.result === void 0 ? null : String(row.result),
		...
		requirementId: row.requirement_id === null || row.requirement_id === void 0 ? "" : String(row.requirement_id),
		branchId: row.branch_id === null || row.branch_id === void 0 ? null : String(row.branch_id),
		retryCount: row.retry_count === null || row.retry_count === void 0 ? 0 : Number(row.retry_count),
		createdAt: iso(row.created_at),
		updatedAt: iso(row.updated_at)
	};
}
...
var RequirementsRepo = class {
	...
	async list(options) {
		await this.ready;
		return (await this.pgmas.withClient(this.database, (client) => client.query(`
        ...
        limit 500
      `, [options?.projectId ?? null]))).rows.map((row) => ({
			...rowToView(row),
	...
	*/
	async create(title, description, projectId) {
		...
		const status = projectId === void 0 ? "open" : "draft";
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`insert into requirements (id, user_id, title, description, status, project_id, created_at, updated_at)
         ...
         returning id, title, description, status, project_id, created_at, updated_at`, [
			...
			status,
			projectId === void 0 ? null : projectId
		]))).rows[0];
		if (row === void 0) throw new Error("插入需求失败：没有返回行");
	...
	async transition(id, to) {
		...
		return this.pgmas.withClient(this.database, async (client) => {
			await client.query("begin");
			try {
				const row = (await client.query("select status from requirements where id = $1 for update", [id])).rows[0];
				if (row === void 0) throw new Error("需求不存在或已删除");
				const from = assertStatus(row.status);
				if (!canTransition(from, target)) throw new Error(`非法状态流转 ${from} → ${target}`);
				const updatedRow = (await client.query(`update requirements set status = $1, updated_at = now()
           ...
           returning id, title, description, status, project_id, created_at, updated_at`, [
				...
				if (updatedRow === void 0) throw new Error("需求状态已变化，请刷新后重试");
				if (target === "terminated") await client.query(`update records set status = 'terminated', result = '需求已终止', updated_at = now()
             where requirement_id = $1 and status not in ('success', 'terminated')`, [id]);
				await client.query("commit");
	...
	async getById(id) {
		await this.ready;
		const row = (await this.pgmas.withClient(this.database, (client) => client.query("select id, title, description, status, project_id, created_at, updated_at from requirements where id = $1", [id]))).rows[0];
		return row === void 0 ? void 0 : rowToView(row);
	}
	/** 该需求最近一条 success record（供下阶段上下文引用产物）。 */
	async listRecentRecord(requirementId) {
		await this.ready;
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`select id, requirement_id, branch_id, category, status, result, artifacts, skills, parent_id, created_at, updated_at
         from records where requirement_id = $1 and status = 'success'
           and category not in ('review-plan', 'review-code')
         order by created_at desc limit 1`, [requirementId]))).rows[0];
		return row === void 0 ? void 0 : recordRowToView(row);
	}
	/** 该需求某 category 最近一条 record（延后人审门定位被审产物 record 用）。 */
	async latestRecordByCategory(requirementId, category) {
		await this.ready;
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`select id, requirement_id, branch_id, category, status, result, artifacts, skills, parent_id, retry_count, created_at, updated_at
         from records where requirement_id = $1 and category = $2
         order by created_at desc, id desc limit 1`, [requirementId, category]))).rows[0];
		return row === void 0 ? void 0 : recordRowToView(row);
	}
	/** Worker: open a stage ledger row (status `running`). */
	async appendRecord(input) {
		await this.ready;
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`insert into records (id, requirement_id, branch_id, category, title, status, result, artifacts, skills, parent_id, retry_count, created_at, updated_at)
		...
		if (row === void 0) throw new Error("插入记录失败：没有返回行");
	...
	async updateRecord(id, patch) {
		...
		if (patch.status !== void 0) push("status", patch.status);
		if (patch.result !== void 0) push("result", patch.result);
		if (patch.artifacts !== void 0) push("artifacts", patch.artifacts);
		if (patch.skills !== void 0) push("skills", patch.skills);
		if (patch.retryCount !== void 0) push("retry_count", patch.retryCount);
		if (sets.length === 0) throw new Error("updateRecord: 没有要更新的字段");
		sets.push("updated_at = now()");
		values.push(id);
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`update records set ${sets.join(", ")} where id = $${values.length}
         returning id, requirement_id, branch_id, category, status, result, artifacts, skills, parent_id, retry_count, created_at, updated_at`, values))).rows[0];
		if (row === void 0) throw new Error("记录不存在或已删除");
		return recordRowToView(row);
	}
	/** Worker: 标记一次重试——复用原 record（不新开），retry_count 原子 +1。 */
	async markRetrying(id) {
		await this.ready;
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`update records set status = 'retrying', retry_count = retry_count + 1, updated_at = now()
         where id = $1
         returning id, requirement_id, branch_id, category, status, result, artifacts, skills, parent_id, retry_count, created_at, updated_at`, [id]))).rows[0];
		if (row === void 0) throw new Error("记录不存在或已删除");
		return recordRowToView(row);
	}
	...
	async listRecords(filters = {}) {
		...
		const conditions = [];
		const values = [];
		if (filters.category !== void 0 && filters.category !== "") {
			values.push(filters.category);
			conditions.push(`rc.category = $${values.length}`);
		}
		if (filters.requirementId !== void 0 && filters.requirementId !== "") {
		...
		return (await this.pgmas.withClient(this.database, (client) => client.query(`select rc.id, rc.requirement_id, rc.branch_id, rc.category, rc.status, rc.result,
         ...
         limit 500`, values))).rows.map((row) => ({
			...recordRowToView(row),
			requirementTitle: row.requirement_title === null || row.requirement_title === void 0 ? null : String(row.requirement_title)
		}));
	}
	/** 运行页：删除一条 record（其 ask_user_questions 由 ON DELETE CASCADE 清理）。 */
	async removeRecord(id) {
		await this.ready;
		await this.pgmas.withClient(this.database, (client) => client.query("delete from records where id = $1", [id]));
	}
	/** 单条 record + 需求标题（records/update 后返回完整列表项）。 */
	async getRecordListItem(id) {
		await this.ready;
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`select rc.id, rc.requirement_id, rc.branch_id, rc.category, rc.status, rc.result,
                rc.artifacts, rc.skills, rc.parent_id, rc.retry_count, rc.created_at, rc.updated_at,
                rq.title as requirement_title
         from records rc
         left join requirements rq on rq.id::text = rc.requirement_id
	...
	async updateRequirement(id, patch) {
		...
		if (patch.description !== void 0) push("description", patch.description === null ? null : String(patch.description));
		if (patch.projectId !== void 0) push("project_id", patch.projectId === null ? null : patch.projectId);
		if (sets.length === 0) throw new Error("updateRequirement: 没有要更新的字段");
		sets.push("updated_at = now()");
		values.push(id);
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`update requirements set ${sets.join(", ")} where id = $${values.length}
         returning id, title, description, status, project_id, created_at, updated_at`, values))).rows[0];
		if (row === void 0) throw new Error("需求不存在或已删除");
		return rowToView(row);
	}
...
var ProjectsRepo = class {
	...
	async list() {
		await this.ready;
		return (await this.pgmas.withClient(this.database, (client) => client.query("select id, name, local_path, git_url, platform, pr_token from projects order by created_at asc, name asc"))).rows.map((row) => ({
			id: String(row.id),
			name: String(row.name),
			localPath: String(row.local_path),
			gitUrl: String(row.git_url),
			platform: row.platform === "gitea" ? "gitea" : "gitee",
			hasToken: row.pr_token !== null && row.pr_token !== void 0 && String(row.pr_token) !== ""
		}));
	}
	async create(input) {
		await this.ready;
		...
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`insert into projects (id, name, local_path, git_url, platform, pr_token, created_at, updated_at)
         values (gen_random_uuid(), $1, $2, $3, $4, $5, now(), now())
         returning id, name, local_path, git_url, platform, pr_token`, [
			...
			gitUrl,
			input.platform,
			input.prToken === void 0 || input.prToken === "" ? null : input.prToken
		]))).rows[0];
		if (row === void 0) throw new Error("插入项目失败：没有返回行");
		return {
			id: String(row.id),
			name: String(row.name),
			localPath: String(row.local_path),
	...
	async getToken(id) {
		await this.ready;
		const token = (await this.pgmas.withClient(this.database, (client) => client.query("select pr_token from projects where id = $1", [id]))).rows[0]?.pr_token;
		return typeof token === "string" && token !== "" ? token : void 0;
	}
	/** Resolve one project by id (worker 领取后取项目信息)。 */
	async getById(id) {
		await this.ready;
		const row = (await this.pgmas.withClient(this.database, (client) => client.query("select id, name, local_path, git_url, platform, pr_token from projects where id = $1", [id]))).rows[0];
	...
	async update(id, patch) {
		...
		if (patch.localPath !== void 0) {
			const localPath = String(patch.localPath).trim();
			if (localPath === "") throw new Error("本地路径不能为空");
			push("local_path", localPath);
		}
		if (patch.gitUrl !== void 0) {
			const gitUrl = String(patch.gitUrl).trim();
			if (gitUrl === "") throw new Error("git 链接不能为空");
			push("git_url", gitUrl);
		}
		if (patch.platform !== void 0) {
			if (patch.platform !== "gitee" && patch.platform !== "gitea") throw new Error("平台必须是 gitee 或 gitea");
			push("platform", patch.platform);
		}
		if (patch.prToken !== void 0) push("pr_token", patch.prToken === null || patch.prToken === "" ? null : patch.prToken);
		if (sets.length === 0) throw new Error("update: 没有要更新的字段");
		sets.push("updated_at = now()");
		values.push(id);
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`update projects set ${sets.join(", ")} where id = $${values.length}
         returning id, name, local_path, git_url, platform, pr_token`, values))).rows[0];
		if (row === void 0) throw new Error("项目不存在或已删除");
		return {
			id: String(row.id),
			name: String(row.name),
			localPath: String(row.local_path),
	...
	async remove(id) {
		...
		await this.pgmas.withClient(this.database, async (client) => {
			...
			const n = Number(refs.rows[0]?.n ?? 0);
...
var QuestionsRepo = class {
	...
	async insertMany(recordId, questions) {
		await this.ready;
		await this.pgmas.withClient(this.database, async (client) => {
			for (const question of questions) await client.query(`insert into ask_user_questions (id, record_id, question, options, status, created_at)
	...
	async listByRecord(recordId) {
		await this.ready;
		return (await this.pgmas.withClient(this.database, (client) => client.query(`select id, record_id, question, options, status, answer, created_at, answered_at
         from ask_user_questions where record_id = $1 order by created_at asc, id asc`, [recordId]))).rows.map(questionRowToView);
	}
	async pendingByRecord(recordId) {
		await this.ready;
		return (await this.pgmas.withClient(this.database, (client) => client.query(`select id, record_id, question, options, status, answer, created_at, answered_at
         from ask_user_questions where record_id = $1 and status = 'pending' order by created_at asc, id asc`, [recordId]))).rows.map(questionRowToView);
	}
	async answer(questionId, answer) {
		await this.ready;
		const trimmed = (answer ?? "").trim();
		if (trimmed === "") throw new Error("回答不能为空");
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(`update ask_user_questions set status = 'answered', answer = $2, answered_at = now()
         where id = $1
         returning id, record_id, question, options, status, answer, created_at, answered_at`, [questionId, trimmed]))).rows[0];
		if (row === void 0) throw new Error("问题不存在或已删除");
		return questionRowToView(row);
	}
};
/** 审核单账本：人工审核门（review）+ 待决策放行审核（reply）。 */
var ReviewsRepo = class {
	database;
	pgmas;
	ready;
	constructor(options) {
	...
	async create(recordId, kind) {
		await this.ready;
		await this.pgmas.withClient(this.database, (client) => client.query(`insert into reviews (id, record_id, kind, status, created_at)
         values (gen_random_uuid(), $1, $2, 'pending', now())`, [recordId, kind]));
		const latest = await this.latestByRecord(recordId);
		if (latest === void 0) throw new Error("插入审核单失败：没有返回行");
		return latest;
	}
	/** 幂等补单：record 尚无 pending reply 单时插入一张（旧 waiting_reply 数据兼容；驳回重跑后再提问也会补新单）。 */
	async ensureReply(recordId) {
		await this.ready;
		const latest = await this.latestByRecord(recordId);
		if (latest !== void 0 && latest.kind === "reply" && latest.status === "pending") return;
		await this.pgmas.withClient(this.database, (client) => client.query(`insert into reviews (id, record_id, kind, status, created_at)
         values (gen_random_uuid(), $1, 'reply', 'pending', now())`, [recordId]));
	}
	/** 审核大厅：所有 pending 审核单（含关联 record/需求信息）；已终止需求的不再展示。 */
	async listPending() {
		await this.ready;
		return (await this.pgmas.withClient(this.database, (client) => client.query(REVIEW_JOIN_SQL + `
        where v.status = 'pending'
          and (rq.status is null or rq.status <> 'terminated')
        order by v.created_at asc, v.id asc
        limit 200
      `))).rows.map(reviewRowToView);
	}
	/** 某 record 的最新一张审核单（无则 undefined）。 */
	async latestByRecord(recordId) {
		await this.ready;
		const row = (await this.pgmas.withClient(this.database, (client) => client.query(REVIEW_JOIN_SQL + `
        where v.record_id = $1
        order by v.created_at desc, v.id desc
        limit 1
      `, [recordId]))).rows[0];
		return row === void 0 ? void 0 : reviewRowToView(row);
	}
	...
	async listByRecord(recordId) {
        ...
        where v.record_id = $1
        order by v.created_at asc, v.id asc
      `, [recordId]))).rows.map(reviewRowToView);
	}
	/** 面板：审核通过。reply 放行单必须全部问题作答完毕才能通过（与审核大厅规则一致）。 */
	async approve(id) {
		await this.ready;
		await this.pgmas.withClient(this.database, async (client) => {
			if ((await client.query("select kind from reviews where id = $1", [id])).rows[0]?.kind === "reply") {
				if (((await client.query(`select 1 from ask_user_questions q
           join reviews v on v.record_id = q.record_id
           where v.id = $1 and q.status = 'pending' limit 1`, [id])).rows ?? []).length > 0) throw new Error("仍有未作答的问题，请全部答完后再审核通过");
			}
			await client.query(`update reviews set status = 'approved', decided_at = now()
	...
	async reject(id, feedback) {
		...
		await this.pgmas.withClient(this.database, (client) => client.query(`update reviews set status = 'rejected', feedback = $2, decided_at = now()
         where id = $1 and status = 'pending'`, [id, trimmed]));
		return this.requireById(id);
	}
	async requireById(id) {
        ...
        limit 1
      `, [id]))).rows[0];
		if (row === void 0) throw new Error("审核单不存在或已删除");
...
const REVIEW_JOIN_SQL = `
  ...
  left join requirements rq on rq.id::text = rc.requirement_id
`;
function reviewRowToView(row) {
	return {
		id: String(row.id),
		recordId: String(row.record_id),
		kind: row.kind === "reply" ? "reply" : "review",
		status: row.status === "approved" ? "approved" : row.status === "rejected" ? "rejected" : "pending",
		feedback: row.feedback === null || row.feedback === void 0 ? null : String(row.feedback),
		createdAt: iso(row.created_at),
		decidedAt: row.decided_at === null || row.decided_at === void 0 ? null : iso(row.decided_at),
		category: String(row.category),
		result: row.result === null || row.result === void 0 ? null : String(row.result),
		artifacts: toTextArray(row.artifacts),
		requirementId: row.requirement_id === null || row.requirement_id === void 0 ? "" : String(row.requirement_id),
...
function questionRowToView(row) {
	return {
		...
		answer: row.answer === null || row.answer === void 0 ? null : String(row.answer),
		createdAt: iso(row.created_at),
		answeredAt: row.answered_at === null || row.answered_at === void 0 ? null : iso(row.answered_at)
	};
}
/** Worker 运行配置存储：worker_config 单例行（id=1）。 */
var WorkerConfigRepo = class {
	database;
	pgmas;
	ready;
	constructor(options) {
	...
	async get() {
		await this.ready;
		const row = (await this.pgmas.withClient(this.database, (client) => client.query("select payload from worker_config where id = 1"))).rows[0];
	...
	async set(config) {
		...
		await this.pgmas.withClient(this.database, (client) => client.query(`insert into worker_config (id, payload, updated_at)
         values (1, $1::jsonb, now())
         on conflict (id) do update set payload = excluded.payload, updated_at = now()`, [JSON.stringify(normalized)]));
		return normalized;
	}
};
//#endregion
