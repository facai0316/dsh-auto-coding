 ...
 */
export const REVIEW_GATED = ['decision'];
export const DEFERRED_REVIEW_GATES = [
 ...
 */
export function withinWindow(config, now = new Date()) {
    if (config.timeWindowEnabled !== true)
        return true;
 ...
 */
export function stageWindowAllowed(config, category, now = new Date()) {
    if (config.timeWindowEnabled !== true)
        return true;
    const limited = config.timeWindowStages;
    if (limited === null || limited === undefined)
        return withinWindow(config, now);
    if (!limited.includes(category))
        return true;
    return withinWindow(config, now);
}
/**
 ...
 */
export async function runLanes(count, run) {
...
export function parsePrResult(value) {
    if (value === null || typeof value !== 'object')
        return null;
    const v = value;
    if (v.is_ok !== true && v.is_ok !== 'true' && v.is_ok !== false && v.is_ok !== 'false')
        return null;
    return {
 ...
 */
export function buildPrPrompt(input) {
    return [
...
export function buildResolvePrompt(input) {
    ...
    if (input.userAnswers.length > 0) {
        ...
        for (const answer of input.userAnswers)
            lines.push(`Q: ${answer.question}  A: ${answer.answer}`);
    }
 ...
 * 只校验不含空白的相对路径条目。
 */
export function isPathArtifact(entry) {
 ...
 */
export async function missingArtifacts(wtPath, artifacts, exists) {
    const missing = [];
    for (const entry of artifacts ?? []) {
        if (!isPathArtifact(entry))
            continue;
        if (!(await exists(wtPath, entry)))
            missing.push(entry);
    }
    return missing;
}
...
export function buildPrompt(input) {
    ...
    if (input.feedback !== undefined && input.feedback.trim() !== '') {
        lines.push('', '# 审核整改意见（驳回重跑）', `上一版产物未通过人工审核，以下整改意见必须逐条落实：`, input.feedback.trim(), '请基于已有产物修订（不要推翻需求），完成后照常返回结构化结果。');
    }
...
export function parseStageResult(value) {
    if (value === null || typeof value !== 'object')
        return null;
    const v = value;
...
export class WorkerPipeline {
    ...
    async runPipeline(input, opts) {
        ...
        for (let i = startIndex; i < STAGES.length; i++) {
            ...
            if (outcome === 'waiting' || outcome === 'failed' || outcome === 'terminated' || outcome === 'deferred')
                return outcome;
        }
    ...
    async runStage(requirement, stage, opts) {
        ...
        if (this.deps.commitWorktree !== undefined) {
            try {
                await this.deps.commitWorktree({ id: requirement.project.id, localPath: requirement.project.localPath }, requirement.wt.path, `${stage.category}(pipeline): 阶段产物提交 (req-${requirement.id.slice(0, 8)})`);
            }
            catch (error) {
    ...
    async startResolve(requirementId) {
        ...
        if (requirement.status !== 'merging')
            throw new Error('只有「待合并」状态的需求可以解决冲突');
        if (requirement.projectId === null)
            throw new Error('需求未关联项目，无法解决冲突');
        const project = await this.deps.projects.getById(requirement.projectId);
        if (project === undefined)
            throw new Error('需求关联的项目不存在');
        ...
        `, [requirementId, branch, [], []]);
                await client.query('commit');
                return { recordId: String(inserted.rows[0].id), started: true };
            }
            catch (error) {
    ...
    async continueAfterGate(row) {
        ...
        if (requirement === undefined || requirement.status !== 'in_progress')
            return;
        if (requirement.projectId === null)
            return;
        const project = await this.deps.projects.getById(requirement.projectId);
    ...
    async resumeRepliedRecord(row) {
        ...
        await this.runPipeline(input, { resume: { recordId: row.record_id, category: row.category, userAnswers: answers } });
    }
    ...
    async rerunWithFeedback(row, feedback) {
        ...
        const expectedStatus = row.category === 'resolve' ? 'merging' : 'in_progress';
        if (requirement === undefined || requirement.status !== expectedStatus)
            return;
        if (requirement.projectId === null)
            return;
        const project = await this.deps.projects.getById(requirement.projectId);
        ...
        // 重试成功后必须续跑链条，否则任务就此结束、无人再推进——需求停在
        // 「阶段 success、后续阶段/merge 无 record」的缺口僵尸态（ADR-025 同类：
        // 表面执行中，实际无运行无审核；只能靠进程重启后的缺口自愈兜底）。
        // - 延后门（plan）被驳回重跑成功 → 重新走机审（review-plan）→ 再次挂人审门；
        // - 其余阶段 → 从下一阶段接续；最后阶段（review-code）→ 直接进入 merge。
        //   人审门不会被绕过：需要挂门的阶段 runStage 返回 'waiting' 而非 'success'，
        //   与 continueAfterGate 的接续语义一致。
        if (outcome === 'success') {
            const deferred = DEFERRED_REVIEW_GATES.find(gate => gate.category === row.category);
            if (deferred !== undefined) {
    ...
    async retryRecord(row) {
        ...
        if (outcome === 'success') {
            ...
            if (nextIndex < STAGES.length) {
                ...
                // 最后阶段（review-code）重试成功 → 直接进入 merge。缺此分支时任务就此
                // 结束、无人再调 runMerge：需求停在「6 阶段全 success、无 merge record」
                // 的缺口僵尸态（ADR-030 现场即 review-code 失败 2 次后重试成功触发）。
                await this.runMerge(input);
            }
        }
    }
    ...
    async markStaleRunning() {
        const res = await this.deps.pgmas.withClient(this.deps.database, client => client.query(`
        update records r
        set status = 'failed',
