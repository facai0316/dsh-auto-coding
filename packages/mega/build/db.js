...
function toJsonSafe(value, depth = 0) {
    ...
    if (Array.isArray(value))
        return depth > 16 ? null : value.map(entry => toJsonSafe(entry, depth + 1));
    if (type === 'object') {
        if (depth > 16)
            return null;
        const out = {};
...
const STRICT_WRITE_PATTERNS = STRICT_WRITE_VERBS
    .map(verb => [verb, new RegExp(`(^|[^A-Za-z0-9_$])${verb}([^A-Za-z0-9_$]|$)`, 'i')]);
 ...
 */
export function writeKeyword(sql) {
...
export function apply(ctx, config) {
    ...
    const allowDatabase = (database) => {
        if (anyDatabases || config.databases.includes(database))
            return database;
        throw new Error(`database "${database}" is not enabled for pg-mas; enabled: ${config.databases.join(', ')} `
    ...
    async function withClient(database, run, signal) {
        ...
        try {
            ...
            try {
                ...
                if (cancel !== undefined && signal !== undefined)
    ...
    const rawQuery = (database, sql, params, signal) => withClient(database, client => params === undefined
        ? client.query({ text: sql })
        : client.query({ text: sql, values: params }), signal);
    ...
    async function query(sql, options = {}) {
        ...
        if (config.readOnly) {
            ...
            if (verb !== '') {
                throw new Error(`pg-mas is mounted read-only and the statement contains the write keyword "${verb}" `
    ...
    async function describeTable(table, database) {
        ...
        if (tableName.length === 0)
            throw new Error(`invalid table reference "${trimmed}"`);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            if (args.listDatabases === true) {
                ...
                return { database: '*', scope: 'databases', databases };
            }
            if (args.table !== undefined) {
                const detail = await describeTable(args.table, args.database);
                return { database: args.database ?? config.database, scope: 'table', table: detail };
            }
            const tables = await listTables(args.database, exec.signal);
            return { database: args.database ?? config.database, scope: 'tables', tables };
        },
        presentCall: (args) => ({
            card: 'generic',
...
function renderSchema(value) {
    if (value.scope === 'databases') {
        ...
        const lines = [
            ...
            ...rows.map(row => `${row.name.padEnd(width)}  ${row.size.padEnd(12)} ${String(row.activeConnections).padStart(3)}${row.default ? '  (default)' : ''}`),
        ];
        return lines.join('\n');
    }
    if (value.scope === 'table' && value.table !== undefined) {
        ...
        const lines = [
            ...
            ...detail.columns.map(column => `  ${column.name.padEnd(columnWidth)}  ${column.type.padEnd(typeWidth)}  ${column.nullable ? 'null' : 'not null'}`
        ...
        if (detail.foreignKeys.length > 0) {
            ...
            for (const foreignKey of detail.foreignKeys)
                lines.push(`  ${foreignKey.definition}`);
        }
        return lines.join('\n');
    }
    const rows = value.tables ?? [];
    if (rows.length === 0)
        return `[pg-mas/${value.database}] (no tables or views)`;
    ...
    const lines = rows.map(row => `${row.schema.padEnd(schemaWidth)}  ${row.name.padEnd(nameWidth)}  ${row.kind.padEnd(kindWidth)}  `
