...
import { STAGES, WorkerPipeline, parsePrResult, stageWindowAllowed, withinWindow } from "./worker-pipeline.js";
export { buildResolvePrompt, buildPrompt, buildPrPrompt, runLanes, stageWindowAllowed, withinWindow, WorkerPipeline } from "./worker-pipeline.js";
...
export default class CmWorkerService extends Service {
        ...
        this.pipeline = new WorkerPipeline({
            ...
            readSkillMd: async (repo, skill) => {
                try {
                    ...
                    if (dir === undefined)
                        throw new Error(`技能 ${skill} 不在项目与 skillsSource 中`);
    ...
    async dispatchClaims() {
        ...
        if (!this.windowFor(STAGES[0].category))
            return;
        while (this.trySlot()) {
            let claim;
            try {
    ...
    async dispatchReviews() {
        ...
        for (const action of actions) {
            ...
            if (action.review_status === 'rejected' || action.review_kind === 'reply') {
                if (!this.windowFor(action.category))
                    continue;
            }
            if (!this.trySlot())
                break;
    ...
    async dispatchRetries() {
        ...
        for (const row of rows) {
            ...
            if (!this.windowFor(row.category))
                continue;
            if (!this.trySlot())
                break;
    ...
    async dispatchGaps() {
        ...
        for (const gap of gaps) {
            if (this.inflight.has(gap.requirement_id))
                continue;
            if (!this.trySlot())
                break;
            this.dispatchTask(() => this.pipeline.resumeGap(gap), `缺口续跑 ${gap.requirement_id.slice(0, 8)}`, undefined, gap.requirement_id);
        }
    }
    /**
