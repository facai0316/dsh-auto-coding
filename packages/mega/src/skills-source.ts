...
export const DEFAULT_SKILLS_SOURCE: SkillsSourceConfig = { kind: 'builtin' }

/** Validate a raw row config value; unknown/invalid kinds fall back to builtin. */
export function normalizeSkillsSource(input: unknown): SkillsSourceConfig {
  if (input === undefined || input === null || typeof input !== 'object') return DEFAULT_SKILLS_SOURCE
...
export class SkillSource {
  constructor(
    private readonly config: SkillsSourceConfig,
  ...
  list(): string[] {
    ...
    try {
      return readdirSync(root).filter(name => {
        try { return existsSync(join(root, name, 'SKILL.md')) } catch { return false }
      })
    } catch {
      return []
    }
  ...
  private rootSync(): string | undefined {
  ...
  private gitRootSync(): string | undefined {
    if (this.config.kind !== 'git') return undefined
    ...
    try {
      ...
      if (this.config.ref !== undefined) args.push('--branch', this.config.ref)
...
export function readSkillMd(source: SkillSource, skill: string): string | undefined {
