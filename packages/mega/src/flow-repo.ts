...
/**
 * Forward migrations. 简化（ADR-032）：默认全部是新库——一套 `create table
 * if not exists` 建表 SQL 从零建全量表，无外键（关联列建索引保性能）。
 * v1-v8 历史版本号全部 no-op 占位（旧库 `_cm_flow_migrations` 已记录的
 * 版本直接跳过；新库从 v8 起步全量建表）。
 */
interface Migration {
  version: number
  name: string
...
const MIGRATIONS: readonly Migration[] = [
  ...[1, 2, 3, 4, 5, 6, 7].map(version => ({
    ...
    apply: async (_client: PoolClient): Promise<void> => {},
  })),
  {
    version: 8,
 ...
 */
export async function runMigrations(pgmas: WriteSeam, database: string, _userId: string): Promise<string[]> {
