...
export class RequirementsRepo {
  ...
  async appendRecord(input: RecordInput): Promise<RecordView> {
    ...
    const result = await this.pgmas.withClient(this.database, client =>
      client.query(
        `insert into records (id, requirement_id, branch_id, category, title, status, result, artifacts, skills, parent_id, created_at, updated_at)
  ...
  async updateRecord(
    id: string,
    patch: { status?: RecordStatus; result?: string; artifacts?: string[]; skills?: string[] },
  ): Promise<RecordView> {
    await this.ready
    const sets: string[] = []
